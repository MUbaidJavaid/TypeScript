var unionOfvariable: string | number | null | [] | {} | undefined = "union";
...
function unionFunction(param: string | number | boolean): string {
...
var unionArray: (string | number)[] = ["Hello", 42, "World"];
...
function unionArrayFunction(arr: (string | number)[]): string {
  return arr.map(item => {
...
var unionObject: { name: string; age: number } | { title: string; year: number } = {
    name: "Alice",
    age: 30
    };
...
function unionObjectFunction(obj: { name?: string; age?: number; title?: string; year?: number }): string {
